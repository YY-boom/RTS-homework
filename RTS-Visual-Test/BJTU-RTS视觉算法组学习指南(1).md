...
非常高兴你对北京交通大学RTS机器人队的视觉算法组感兴趣，在这篇指南接下来的部分中，我们将会逐步引导你开始学习进入我们组之前需要储备的一些知识与会用到的工具，在学习的过程中有任何疑问都欢迎来与我们进行探讨。

## 说明

+ **任务提交截止：**

  > <div style = "color: blue;">日期暂未确定</div>
  >
+ 本学习指南中的考核题作为学习成果的检验与实践，在正式入队前每位同学都需要完成。
+ 关于生成式人工智能模型：我们**完全不限制**大家对生成式模型的使用，并且鼓励大家逐渐摸索出一个适合于自己的使用生成式模型的方法来提升自己的学习新知识的效率，~~以及降低debug时的红温程度~~。
+ **关于考核题的提交**：在GitHub上创建自己的仓库，上传任务中要求的材料并为仓库创建一个readme文件，在文件内写上姓名、学号以及仓库结构说明（每个文件夹存放了什么内容），并将仓库链接发送给我。关于GitHub及git的操作，在本指南后续的部分会进行讲解
+ **部分考核所需要的额外资料存放在本仓库的data.zip中**
+ 考核中所要求的各类文档均需要使用markdown语法编写，可以在编写完成后渲染为pdf后再提交
+ **关于各任务中需要提交的材料清单**:将会在**本指南末尾**进行汇总,每个task中将仅叙述任务内容

🌟好的，那么接下来就让我们开始这一次的学习之旅~

## ***PART1：一切的开始：Linux***

在我们组的工作中，我们将不再使用Windows，而是选择Linux作为主要的开发环境，再这个赛季，我们将逐步从ROS1过渡到ROS2，所以对于系统版本的选择可以使用更加现代化的版本，但为了获得更好的兼容性，Ubuntu20与Ubuntu22仍是比较不错的选择。

安装Ubuntu系统目前有三种主流的方案，三种方案各有优劣，在前期的学习中大家可以任意选择，但是在正式入队之后建议更换为双系统来获取与硬件连接时更好的稳定性

+ 方案一：虚拟机
  + [通过vmware安装Ubuntu](https://blog.csdn.net/qq_62888264/article/details/145102532)
+ 方案二：WSL
  + [Ubuntu图形化界面安装 ](https://www.bilibili.com/opus/915821060589355009)
+ 方案二：双系统
  + [实体机安装双系统](https://hs.cnies.org/archives/dual-boot-ubuntu2404-win11)

> <div style="color: green;">Tip1：</div>
> 在安装双系统时，建议为Linux分区保留100G以上的空间，才会在后续的工作开展时不会显得捉襟见肘。当然，你也可以选择暂时先分配40G左右的空间给Linux用于完成本指南中的任务，后续条件允许时再重新为Linux分配更大的空间。~~重装系统在进行我们组的工作时十分常见，因为在具体工作开展的过程中，操作不当造成环境污染是一个很难避免的问题，这也体现出git的重要性~~

> <div style="color: green;"> Tip2： </div>
> 如果电脑的剩余空间已经无法分配足够的空间用于安装Linux，可以采用外接硬盘的方式安装，操作流程与相应的教程一致，若采用wsl方式则需要[额外的操作](https://www.sysgeek.cn/move-wsl-distros-windows/)
...
> Info
> 这部分所使用的rosbag包由于GitHub文件体积限制，故无法上传至GitHub仓库，获取方式及aruco标识（例子）均放到了aruco-data文件夹下
...
+ Task8：
  ...
  - 编写的所有功能包
  - 运行时截图
  - 一个解释文档（描述这两个包都实现了什么功能）

  > <div style = "color: red;">Important！</div>
  > 所有文件的提交均需要提交到GitHub的个人仓库，并将仓库链接提交到微信群中腾讯文档中。
  >
